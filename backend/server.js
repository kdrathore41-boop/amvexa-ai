...
function detectIntent(message) {
  ...
  if (/\b(remember|save|store|note|yaad rakh(?:o|na)?)\b/.test(text) || /^\s*(mera naam|my name)\s+(?!kya\b|what\b).+/i.test(text) || /\b(hamesha|always)\b.*\b(hindi|हिंदी)\b/.test(text) || /\b(hindi|हिंदी)\b.*\b(hamesha|always)\b/.test(text)) return "memory";
  ...
  if (/\b(add|create|creat|make|set|new)\s+(a\s+)?(task|tast|todo)\b/.test(text)) return "planning";
...
function taskFromMessage(message) { return message.replace(/^\s*(add|create|creat|make|set|new)\s+(a\s+)?(task|tast|todo)\s*[:,-]?\s*/i, "").trim(); }
...
function planTool(message){ const intent=detectIntent(message); if(intent==="memory"){const content=/\b(hindi|हिंदी)\b/i.test(message)?"Mujhe hamesha Hindi mein jawab dena hai.":extractMemory(message);return {tool:"save_memory",args:{content,kind:/\b(hindi|हिंदी)\b/i.test(message)?"preference":"saved-memory"}};} if(intent==="recall")return {tool:"recall_memory",args:{query:message}}; if(intent==="task_complete")return {tool:"complete_task",args:{reference:message}}; if(intent==="music")return {tool:"music_search",args:{query:message}}; if(intent==="tasks")return {tool:"get_tasks",args:{}}; if(intent==="planning"){const title=taskFromMessage(message); if(/\b(add|create|creat|make|set|new)\s+(a\s+)?(task|tast|todo)\b/i.test(message)&&title)return {tool:"create_task",args:{title,priority:/\b(high|urgent|important|jaruri|zaroori)\b/i.test(message)?"high":"normal"}}; return {tool:"get_daily_plan",args:{}};} if(intent==="research"||/\b(news|khabar|today|aaj|latest|current|recent|source|sources|date|tarikh|internet|web|online)\b/i.test(message))return {tool:"web_search",args:{query:message}}; return {tool:null,args:{}}; }
...
app.post("/api/chat", async (req,res)=>{
  ...
  return res.json({success:true,response:responseText,tool:plan.tool||null,verification});
});
